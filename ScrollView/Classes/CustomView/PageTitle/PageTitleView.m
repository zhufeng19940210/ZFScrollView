...
//
#define zfPageTitleViewWidth self.frame.size.width
#define zfPageTitleViewHeight self.frame.size.height
#import "PageTitleView.h"
#import "UIView+Frame.h"
#import "PageTitleViewConfigura.h"
@interface zfPageTitleButton : UIButton
@end
@implementation zfPageTitleButton
...
/// zfPageTitleViewDelegate
@property (nonatomic, weak) id<PageTitleViewDelegate> delegatePageTitleView;
/// zfPageTitleViewConfigure
@property (nonatomic, strong) PageTitleViewConfigura *configure;
/// scrollView
...
- (instancetype)initWithFrame:(CGRect)frame
                     delegate:(id<PageTitleViewDelegate>)delegate
                   titleNames:(NSArray *)titleNames
                    configure:(PageTitleViewConfigura *)configure {
    if (self = [super initWithFrame:frame]) {
        self.backgroundColor = [[UIColor whiteColor] colorWithAlphaComponent:0.77];
        if (delegate == nil) {
            @throw [NSException exceptionWithName:@"zfPagingView" reason:@"zfPageTitleView 的代理方法必须设置" userInfo:nil];
        }
        self.delegatePageTitleView = delegate;
        if (titleNames == nil) {
            @throw [NSException exceptionWithName:@"zfPagingView" reason:@"zfPageTitleView 的标题数组必须设置" userInfo:nil];
        }
        self.titleArr = titleNames;
        if (configure == nil) {
            @throw [NSException exceptionWithName:@"zfPagingView" reason:@"zfPageTitleView 的配置属性必须设置" userInfo:nil];
        }
...
+ (instancetype)pageTitleViewWithFrame:(CGRect)frame
                              delegate:(id<PageTitleViewDelegate>)delegate
                            titleNames:(NSArray *)titleNames
                             configure:(PageTitleViewConfigura *)configure {
...
- (UIScrollView *)scrollView {
    if (!_scrollView) {
        ...
        _scrollView.frame = CGRectMake(0, 0, zfPageTitleViewWidth, zfPageTitleViewHeight);
    }
    return _scrollView;
}
...
- (UIView *)indicatorView {
    if (!_indicatorView) {
        ...
        if (self.configure.indicatorStyle == SGIndicatorStyleDefault) {
            ...
            _indicatorView.zf_height = indicatorViewH;
            _indicatorView.zf_y = self.zf_height - indicatorViewH;
        } else {
            CGFloat tempIndicatorViewH = [self zf_heightWithString:[self.btnMArr[0] currentTitle] font:self.configure.titleFont];
            if (self.configure.indicatorHeight > self.zf_height) {
                _indicatorView.zf_y = 0;
                _indicatorView.zf_height = self.zf_height;
            } else if (self.configure.indicatorHeight < tempIndicatorViewH) {
                _indicatorView.zf_y = 0.5 * (self.zf_height - tempIndicatorViewH);
                _indicatorView.zf_height = tempIndicatorViewH;
            } else {
                _indicatorView.zf_y = 0.5 * (self.zf_height - self.configure.indicatorHeight);
                _indicatorView.zf_height = self.configure.indicatorHeight;
            }
            
            // 圆角处理
            if (self.configure.indicatorCornerRadius > 0.5 * _indicatorView.zf_height) {
                _indicatorView.layer.cornerRadius = 0.5 * _indicatorView.zf_height;
            } else {
...
- (UIView *)bottomSeparator {
    if (!_bottomSeparator) {
        ...
        CGFloat bottomSeparatorW = self.zf_width;
        CGFloat bottomSeparatorH = 0.5;
        CGFloat bottomSeparatorX = 0;
        CGFloat bottomSeparatorY = self.zf_height - bottomSeparatorH;
...
#pragma mark - - - 计算字符串宽度
- (CGFloat)zf_widthWithString:(NSString *)string font:(UIFont *)font {
    NSDictionary *attrs = @{NSFontAttributeName : font};
    return [string boundingRectWithSize:CGSizeMake(0, 0) options:NSStringDrawingUsesLineFragmentOrigin attributes:attrs context:nil].size.width;
}
- (CGFloat)zf_heightWithString:(NSString *)string font:(UIFont *)font {
    NSDictionary *attrs = @{NSFontAttributeName : font};
...
- (void)setupTitleButtons {
    ...
    [self.titleArr enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        CGFloat tempWidth = [self zf_widthWithString:obj font:self.configure.titleFont];
    ...
    if (self.allBtnWidth <= self.bounds.size.width) { /// zfPageTitleView 不可滚动
        CGFloat btnY = 0;
        CGFloat btnW = zfPageTitleViewWidth / self.titleArr.count;
        CGFloat btnH = 0;
        if (self.configure.indicatorStyle == SGIndicatorStyleDefault) {
            btnH = zfPageTitleViewHeight - self.configure.indicatorHeight;
        } else {
            btnH = zfPageTitleViewHeight;
        }
        for (NSInteger index = 0; index < titleCount; index++) {
            zfPageTitleButton *btn = [[zfPageTitleButton alloc] init];
        ...
        self.scrollView.contentSize = CGSizeMake(zfPageTitleViewWidth, zfPageTitleViewHeight);
        
    } else { /// zfPageTitleView 可滚动
        CGFloat btnX = 0;
        CGFloat btnY = 0;
        CGFloat btnH = 0;
        if (self.configure.indicatorStyle == SGIndicatorStyleDefault) {
        ...
        for (NSInteger index = 0; index < titleCount; index++) {
            zfPageTitleButton *btn = [[zfPageTitleButton alloc] init];
            CGFloat btnW = [self zf_widthWithString:self.titleArr[index] font:self.configure.titleFont] + self.configure.spacingBetweenButtons;
            btn.frame = CGRectMake(btnX, btnY, btnW, btnH);
            btnX = btnX + btnW;
            btn.tag = index;
            btn.titleLabel.font = self.configure.titleFont;
        ...
        self.scrollView.contentSize = CGSizeMake(scrollViewWidth, zfPageTitleViewHeight);
    }
}
...
- (void)P_btn_action:(UIButton *)button {
    ...
    // 2、滚动标题选中居中
    if (self.allBtnWidth > zfPageTitleViewWidth) {
...
- (void)P_selectedBtnCenter:(UIButton *)centerBtn {
    // 计算偏移量
    CGFloat offsetX = centerBtn.center.x - zfPageTitleViewWidth * 0.5;
    if (offsetX < 0) offsetX = 0;
    // 获取最大滚动范围
    CGFloat maxOffsetX = self.scrollView.contentSize.width - zfPageTitleViewWidth;
...
- (void)P_changeIndicatorViewLocationWithButton:(UIButton *)button {
    [UIView animateWithDuration:self.configure.indicatorAnimationTime animations:^{
        CGFloat tempIndicatorWidth = self.configure.indicatorAdditionalWidth + [self zf_widthWithString:button.currentTitle font:self.configure.titleFont];
        if (tempIndicatorWidth > button.zf_width) {
            tempIndicatorWidth = button.zf_width;
        }
        self.indicatorView.zf_width = tempIndicatorWidth;
        self.indicatorView.zf_centerx = button.zf_centerx;
    }];
}
...
#pragma mark - - - 内部方法 --> P_
/// zfPageTitleView 不可滚动 - - - zfIndicatorScrollStyleDefault
- (void)P_smallIndicatorScrollStyleDefaultWithProgress:(CGFloat)progress originalBtn:(UIButton *)originalBtn targetBtn:(UIButton *)targetBtn {
    ...
    CGFloat targetBtnX = CGRectGetMaxX(targetBtn.frame) - [self zf_widthWithString:targetBtn.currentTitle font:self.configure.titleFont] - 0.5 * (self.zf_width / self.titleArr.count - [self zf_widthWithString:targetBtn.currentTitle font:self.configure.titleFont] + self.configure.indicatorAdditionalWidth);
    CGFloat originalBtnX = CGRectGetMaxX(originalBtn.frame) - [self zf_widthWithString:originalBtn.currentTitle font:self.configure.titleFont] - 0.5 * (self.zf_width / self.titleArr.count - [self zf_widthWithString:originalBtn.currentTitle font:self.configure.titleFont] + self.configure.indicatorAdditionalWidth);
    CGFloat totalOffsetX = targetBtnX - originalBtnX;
    
    /// 计算 targetBtn／originalBtn 宽度的差值
    CGFloat targetBtnDistance = (CGRectGetMaxX(targetBtn.frame) - 0.5 * (self.zf_width / self.titleArr.count - [self zf_widthWithString:targetBtn.currentTitle font:self.configure.titleFont]));
    CGFloat originalBtnDistance = (CGRectGetMaxX(originalBtn.frame) - 0.5 * (self.zf_width / self.titleArr.count - [self zf_widthWithString:originalBtn.currentTitle font:self.configure.titleFont]));
    ...
    self.indicatorView.zf_x = originalBtnX + offsetX;
    
    CGFloat tempIndicatorWidth = self.configure.indicatorAdditionalWidth + [self zf_widthWithString:originalBtn.currentTitle font:self.configure.titleFont] + distance;
    if (tempIndicatorWidth >= targetBtn.zf_width) {
        CGFloat moveTotalX = targetBtn.zf_orgin.x - originalBtn.zf_orgin.x;
        CGFloat moveX = moveTotalX * progress;
        self.indicatorView.zf_centerx = originalBtn.zf_centerx + moveX;
    } else {
        self.indicatorView.zf_width = tempIndicatorWidth;
    }
}

/// zfPageTitleView 可滚动 - - - zfIndicatorScrollStyleDefault
- (void)P_indicatorScrollStyleDefaultWithProgress:(CGFloat)progress originalBtn:(UIButton *)originalBtn targetBtn:(UIButton *)targetBtn {
    ...
    CGFloat totalOffsetX = targetBtn.zf_orgin.x - originalBtn.zf_orgin.x;
    /// 计算 targetBtn／originalBtn 宽度的差值
    ...
    CGFloat tempIndicatorWidth = self.configure.indicatorAdditionalWidth + [self zf_widthWithString:targetBtn.currentTitle font:self.configure.titleFont];
    if (tempIndicatorWidth >= targetBtn.zf_width) {
        offsetX = totalOffsetX * progress;
        distance = progress * (totalDistance - totalOffsetX);
        self.indicatorView.zf_x = originalBtn.zf_orgin.x + offsetX;
        self.indicatorView.zf_width = originalBtn.zf_width + distance;
    } else {
        ...
        /// 计算 indicatorView 新的 frame
        self.indicatorView.zf_x = originalBtn.zf_orgin.x + offsetX;
        self.indicatorView.zf_width = originalBtn.zf_width + distance + self.configure.indicatorAdditionalWidth;
    }
}
/// zfPageTitleView 不可滚动 - - - zfIndicatorScrollStyleHalf - zfIndicatorScrollStyleEnd
- (void)P_smallIndicatorScrollStyleHalfEndWithProgress:(CGFloat)progress originalBtn:(UIButton *)originalBtn targetBtn:(UIButton *)targetBtn {
    if (self.configure.indicatorScrollStyle == SGIndicatorScrollStyleDefault) {
        if (progress >= 0.5) {
            ...
            [UIView animateWithDuration:self.configure.indicatorAnimationTime animations:^{
                if (tempIndicatorWidth >= targetBtn.zf_width) {
                    self.indicatorView.zf_width = targetBtn.zf_width;
                } else {
                    self.indicatorView.zf_width = tempIndicatorWidth;
                }
                self.indicatorView.zf_centerx = targetBtn.zf_centerx;
                [self P_changeSelectedButton:targetBtn];
            }];
        } else {
            CGFloat tempIndicatorWidth = self.configure.indicatorAdditionalWidth + [self zf_widthWithString:originalBtn.currentTitle font:self.configure.titleFont];
            [UIView animateWithDuration:self.configure.indicatorAnimationTime animations:^{
                if (tempIndicatorWidth >= targetBtn.zf_width) {
                    self.indicatorView.zf_width = originalBtn.zf_width;
                } else {
                    self.indicatorView.zf_width = tempIndicatorWidth;
                }
                self.indicatorView.zf_centerx = originalBtn.zf_centerx;
...
/// zfPageTitleView 可滚动 - - - zfIndicatorScrollStyleHalf - zfIndicatorScrollStyleEnd
- (void)P_indicatorScrollStyleHalfEndWithProgress:(CGFloat)progress originalBtn:(UIButton *)originalBtn targetBtn:(UIButton *)targetBtn {
    if (self.configure.indicatorScrollStyle == SGIndicatorScrollStyleDefault) {
        if (progress >= 0.5) {
            ...
            CGFloat tempIndicatorWidth = self.configure.indicatorAdditionalWidth + [self zf_widthWithString:originalBtn.currentTitle font:self.configure.titleFont];
            [UIView animateWithDuration:self.configure.indicatorAnimationTime animations:^{
                if (tempIndicatorWidth >= originalBtn.zf_width) {
                    self.indicatorView.zf_width = originalBtn.zf_width;
                } else {
                    self.indicatorView.zf_width = tempIndicatorWidth;
                }
